  ...
  return (
    <ProtectedRoute permission="security.permissions">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--hc-text)]">Asignación de Permisos</h1>
          <p className="text-sm text-[var(--on-surface-variant)] mt-1">Configura los permisos diferenciados para cada rol del sistema.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-[var(--on-surface-variant)] mt-6">
                    ...
                    {mod.permissions.map((perm) => (
                      <div key={perm.key} className="flex items-center justify-between py-1">
                        <div className="flex items-center gap-2">
                          <span className={`text-sm ${perm.disabled ? 'text-slate-300' : 'text-[var(--on-surface-variant)]'}`}>
                            {perm.label}
                          </span>
                          {perm.warning && (
