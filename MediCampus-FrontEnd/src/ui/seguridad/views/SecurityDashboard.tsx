  ...
  return (
    <ProtectedRoute permission="security.dashboard">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Panel de Seguridad</h1>
          <p className="text-sm text-slate-500 mt-1">Monitoreo general del estado de seguridad del sistema</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {KPIS.map((kpi) => (
            <Card key={kpi.label} className="flex flex-col">
              ...
              <span className="text-3xl font-bold text-slate-900 mt-1">{kpi.value}</span>
              <div className="mt-2">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {QUICK_LINKS.map((link) => (
              <Card
                key={link.path}
                className="cursor-pointer hover:border-hc-primary transition-colors"
