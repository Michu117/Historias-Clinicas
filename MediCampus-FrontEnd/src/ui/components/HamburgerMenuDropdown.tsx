import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuthContext } from '../seguridad/context/AuthContext';

export interface NavItem {
  label: string;
  icon: string;
  path: string;
  match: string;
}

const DEFAULT_NAV_ITEMS: NavItem[] = [
  { label: 'Inicio', icon: 'home', path: '/home', match: '/home' },
  { label: 'Agendar Cita', icon: 'calendar', path: '/AgendarCita', match: '/AgendarCita' },
  { label: 'Mis Citas', icon: 'list', path: '/mis-citas', match: '/mis-citas' },
  { label: 'Mi Agenda', icon: 'calendar', path: '/agendas/mi-agenda', match: '/agendas/mi-agenda' },
  { label: 'Historias Clinicas', icon: 'certificate', path: '/historias', match: '/historias' },
  { label: 'Notificaciones', icon: 'notifications', path: '/notificaciones', match: '/notificaciones' },
  { label: 'Reportes', icon: 'reportes', path: '/reportes', match: '/reportes' },
];

interface HamburgerMenuDropdownProps {
  navItems?: NavItem[];
}

const getInitialTheme = (): 'light' | 'dark' => {
  const stored = localStorage.getItem('theme');
  if (stored === 'dark' || stored === 'light') return stored;
  return 'light';
};

export const HamburgerMenuDropdown: React.FC<HamburgerMenuDropdownProps> = ({
  navItems: customNavItems,
}) => {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const [theme, setTheme] = useState<'light' | 'dark'>(getInitialTheme);
  const { isAdmin, user } = useAuthContext();

  const isProfessional = user?.roles?.some(r =>
    ['PROFESIONAL', 'profesional', 'medico', 'psicologo', 'odontologo', 'trabajador_social', 'trabajo_social'].includes(r.nombre)
  ) ?? false;

  useEffect(() => {
    document.documentElement.setAttribute('data-theme', theme);
    localStorage.setItem('theme', theme);
  }, [theme]);

  useEffect(() => {
    if (open) {
      document.body.style.marginLeft = '240px';
      document.body.style.transition = 'margin-left 0.2s ease';
    } else {
      document.body.style.marginLeft = '';
      document.body.style.transition = 'margin-left 0.2s ease';
    }
    return () => {
      document.body.style.marginLeft = '';
    };
  }, [open]);

  const navItems = (customNavItems || DEFAULT_NAV_ITEMS).filter(
    (item) => {
      if (item.path.startsWith('/reportes') && !isAdmin) return false;
      if (item.path.startsWith('/agendas/mi-agenda') && !isProfessional) return false;
      return true;
    }
  );

  const handleNavigate = (path: string) => {
    setOpen(false);
    navigate(path);
  };

  const isActive = (match: string) => window.location.pathname.startsWith(match);

  const handleLogout = () => {
    setOpen(false);
    localStorage.clear();
    window.location.href = '/';
  };

  const renderIcon = (icon: string) => {
    switch (icon) {
      case 'home':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />;
      case 'calendar':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />;
      case 'list':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />;
      case 'certificate':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />;
      case 'notifications':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />;
      case 'reportes':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />;
      case 'dashboard':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />;
      case 'group':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />;
      case 'assignment':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />;
      case 'logout':
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />;
      default:
        return <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />;
    }
  };

  return (
    <>
      {!open && (
        <button
          onClick={() => setOpen(true)}
          className="flex items-center justify-center w-9 h-9 rounded-lg transition-all shrink-0"
          style={{ color: 'white', backgroundColor: 'rgba(0,0,0,0.2)' }}
          onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.35)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.2)'; }}
          aria-label="Abrir menú"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </button>
      )}

      {open && (
        <nav
          className="fixed left-0 top-0 h-full w-60 z-50 flex flex-col py-5 px-3 shadow-xl"
          style={{ backgroundColor: 'var(--surface-container)', borderRight: '1px solid var(--outline-variant)' }}
        >
          <div className="flex items-center justify-between px-3 mb-6">
            <div className="flex items-center gap-3">
              <div
                className="w-9 h-9 rounded-lg flex items-center justify-center text-white font-bold text-sm shrink-0"
                style={{ backgroundColor: 'var(--primary)', color: 'var(--on-primary)' }}
              >
                MC
              </div>
              <span className="font-bold text-sm" style={{ color: 'var(--on-surface)' }}>MediCampus</span>
            </div>
            <button
              onClick={() => setOpen(false)}
              className="flex items-center justify-center w-8 h-8 rounded-lg transition-all"
              style={{ color: 'var(--on-surface-variant)' }}
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'var(--surface-container-high)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; }}
              aria-label="Cerrar menú"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </div>

          <div className="flex flex-col gap-1 flex-1">
            {navItems.map((item) => {
              const active = isActive(item.match);
              return (
                <button
                  key={item.path}
                  onClick={() => handleNavigate(item.path)}
                  className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium rounded-lg transition-all text-left"
                  style={{
                    color: active ? 'var(--on-primary-fixed)' : 'var(--hc-text)',
                    backgroundColor: active ? 'var(--primary-fixed)' : 'transparent',
                    borderLeft: active ? '3px solid var(--on-primary-fixed)' : '3px solid transparent',
                  }}
                  onMouseEnter={(e) => {
                    if (!active) e.currentTarget.style.backgroundColor = 'var(--surface-container-high)';
                  }}
                  onMouseLeave={(e) => {
                    if (!active) e.currentTarget.style.backgroundColor = 'transparent';
                  }}
                >
                  <span style={{ color: 'var(--on-surface-variant)' }} className="shrink-0">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      {renderIcon(item.icon)}
                    </svg>
                  </span>
                  {item.label}
                </button>
              );
            })}
          </div>

          <div className="pt-3 flex flex-col gap-1 border-t" style={{ borderColor: 'var(--outline-variant)' }}>
            <button
              onClick={() => setTheme(t => t === 'light' ? 'dark' : 'light')}
              className="flex items-center gap-3 px-4 py-2.5 w-full text-sm font-medium rounded-lg transition-all text-left"
              style={{ color: 'var(--on-surface-variant)' }}
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'var(--surface-container-high)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; }}
            >
              <span className="shrink-0" style={{ color: 'var(--on-surface-variant)' }}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {theme === 'dark' ? (
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                  ) : (
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                  )}
                </svg>
              </span>
              {theme === 'dark' ? 'Modo Claro' : 'Modo Oscuro'}
            </button>
            <button
              onClick={handleLogout}
              className="flex items-center gap-3 px-4 py-2.5 w-full text-sm font-medium rounded-lg transition-all text-left"
              style={{ color: 'var(--primary)' }}
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'var(--surface-container-high)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; }}
            >
              <span className="shrink-0" style={{ color: 'var(--primary)' }}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {renderIcon('logout')}
                </svg>
              </span>
              Cerrar Sesión
            </button>
          </div>
        </nav>
      )}
    </>
  );
};
