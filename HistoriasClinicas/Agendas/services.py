...
def validar_choque_citas(profesional_id, fecha_hora, duracion_minutos=60, excluir_cita_id=None):
    """Valida que un profesional no tenga citas superpuestas."""
    ...
    if citas_conflictivas.exists():
        raise ConflictoHorarioError(
            f'El profesional {profesional_id} ya tiene una cita en ese horario.'
        )
...
def gestionar_derivacion(paciente_id, remitente_id, destinatario, tipo_derivacion, motivo):
    """Crea una derivación interna o externa para un paciente."""
    if not paciente_id:
        raise DatosInvalidosError('Debe proporcionar paciente_id.')
    if not remitente_id:
